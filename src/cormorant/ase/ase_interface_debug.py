...
from cormorant.models.cormorant_ase_debug import CormorantASEDebug
...
from cormorant.data.prepare.process import process_ase, process_db_row_debug, _process_structure
...
class ASEInterfaceDebug(Calculator):
    ...
    def train(self, database, workdir=None, force_factor=0., num_epoch=256, num_channels=3, lr_init=5.e-4, lr_final=5.e-6, batch_size=20, label=None):
        ...
        #num_train = 10
        num_train, num_valid, num_test, datasets, num_species, max_charge = self.initialize_database(num_train, args.num_valid, args.num_test, args.datadir, database, cutoff= args.hard_cut_rad, force_train=force_train)
    ...
    def initialize_database(self, num_train, num_valid, num_test, datadir, database, cutoff, splits=None, force_train=True):
        """
        ...
        for split, split_idx in splits.items():
            process_row = partial(process_db_row_debug, cutoff=cutoff)
            ase_data[split] = process_ase(database, process_row, file_idx_list=split_idx, force_train=force_train)

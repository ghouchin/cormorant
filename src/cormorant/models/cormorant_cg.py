...
class CormorantCG(CGModule):
    ...
    def forward(self, atom_reps, atom_mask, edge_features, edge_mask, rad_funcs, norms, sq_norms, sph_harm):
        """
        ...
        edge_features : SO3 Scalar or None`
        ...
        for idx, (atom_level, edge_level, max_sh) in enumerate(zip(self.atom_levels, self.edge_levels, self.max_sh)):
            edge_features = edge_level(edge_features, atom_reps, rad_funcs[idx], edge_mask, norms, sq_norms)
            edge_reps = edge_features * sph_harm[:max_sh+1]
            atom_reps = atom_level(atom_reps, edge_reps, atom_mask)
            atoms_all.append(atom_reps)
            edges_all.append(edge_features)
